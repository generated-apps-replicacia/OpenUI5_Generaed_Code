...
sap.ui.define([
	 ...
	 **/
	var cellIndexes = { };
				cellIndexes.CourseName = 0;		
				cellIndexes.CourseId = 1;		
		

	return Controller.extend("com.devplatform.config.controller.Course", {
		/**
		...
		onInit : function () {
			...
			this.mGroupFunctions = {
					CourseName: function(oContext) {
						var name = oContext.getProperty("CourseName");
						return {
							key: name,
							text: name
						};
					},					
					CourseId: function(oContext) {
						var name = oContext.getProperty("CourseId");
						return {
							key: name,
							text: name
						};
		...
		_getSelectedItemValues: function(){
			var selectedItem = this.byId( "coursesTable" ).getSelectedItem();
			...
			_activeRow.CourseName = this._getValueType( this._getUIControlValue( cells[cellIndexes.CourseName] ) );
			_activeRow.CourseId = this._getValueType( this._getUIControlValue( cells[cellIndexes.CourseId] ) );
		 ...
		 */
		onCreate : function () {
			var oList = this.byId( "coursesTable" );
		 ...
		 */
		onDelete : function () {
			var oSelected = this.byId( "coursesTable" ).getSelectedItem();
		...
		_selectRowOfChange: function(oEvt){
			...
			this.byId( "coursesTable" ).setSelectedItemById( oFieldId, true);
		},
		...
		onInputChange : function (oEvt) {
			...
			if (oEvt.getParameter("escPressed")) {
				...
				if (oEvt.getSource().getParent().getBindingContext().getProperty("CourseId")) {
		...
		onInputChange_new : function (oEvt) {
			...
			odataServiceURL = odataServiceURL + "Courses(" + itemId + ")";
			/**
		 ...
		 */
		onRefresh : function () {
			var oBinding = this.byId( "coursesTable" ).getBinding("items");
		...
		onResetChanges : function () {
			this.byId( "coursesTable" ).getBinding("items").resetChanges();
		...
		onSearch : function () {
			var oView = this.getView(),
				sValue = oView.byId("searchFieldCourses").getValue(),
				oFilter = new Filter("CourseName", FilterOperator.Contains, sValue);

			oView.byId( "coursesTable" ).getBinding("items").filter(oFilter, FilterType.Application);
		},
		...
		onSort_old : function () {
			...
			oView.byId( "coursesTable" ).getBinding("items").sort(sOrder && new Sorter("description", sOrder === "desc"));
		...
		handleSortButtonPressed: function () {
			this.createViewSettingsDialog("com.devplatform.config.view.SortDialog_Course").open();
		},
		...
		handleGroupButtonPressed: function () {
			this.createViewSettingsDialog("com.devplatform.config.view.GroupDialog_Course").open();
		},
		...
		onMessageBindingChange : function (oEvent) {
			...
			MessageBox.error(aMessages[0].message, {
				id : "serviceErrorMessageBox_Courses",
				onClose : function () {
					bMessageOpen = false;
				}
			});
